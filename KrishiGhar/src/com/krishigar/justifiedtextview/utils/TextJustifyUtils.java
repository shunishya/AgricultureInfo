package com.krishigar.justifiedtextview.utils;
...
public class TextJustifyUtils {
	...
	public static void justify(TextView textView) {
		...
		for (int i = 0; i < blocks.length; i++) {
			...
			wrappedObj = TextJustifyUtils.createWrappedLine(block, paint,
					spaceOffset, textWrapWidth, true, "*");
	...
	protected static Object[] createWrappedLine(String block, Paint paint,
			float spaceOffset, float maxWidth, boolean hyphenate,
		...
		wordsLoop: for (String dirtyWord : block.split("\\s")) {
			if (hyphenate) {
			...
			if (maxWidth <= 0) // Full word doesn't fit in the line, try
								// syllables
			{
				if (hyphenate) {
				...
				return new Object[] { line,
						maxWidth + cacheWidth + spaceOffset, charCounter };
			}
		...
		{
			return new Object[] { line.substring(0, line.length() - 1),
					Float.MIN_VALUE, charCounter - 1 };
		}
